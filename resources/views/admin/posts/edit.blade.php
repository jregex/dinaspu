                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('posts.update', ['post' => $post->id]) }}" method="post"
                                ...
                                <x-adminlte-select2 name="category_id">
                                    @foreach ($categories as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $item->id == $post->category_id ? 'selected' : '' }}>{{ $item->category }}
                                        </option>
                                    @endforeach
                                </x-adminlte-select2>
                            </div>
                            <div class="col-md-6 col-sm-12">
                                ...
                                <div class="input-group mb-3">
                                    <input type="text" name="penulis"
                                        ...
                                        placeholder="Input penulis" value="{{ $post->penulis }}">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="editor" id="editor" rows="15" cols="5">{!! $post->body !!}</textarea>
                        </div>

                        <div class="form-group">
                            <img id="previewImg" src="{{ asset('storage/public/berita') }}/{{ $post->image }}"
                                width="250" height="250" class="img-fluid img-thumbnail mb-5" alt="">
                            <div class="custom-file mb-2">
                                <input type="file" class="form-control @error('profile') is-invalid @enderror"
                                    id="profile" name="profile" lang="en" value="{{ $post->image }}">
                                @error('profile')
